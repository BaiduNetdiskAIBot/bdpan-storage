...
1. **自动更新检查**（每次触发时静默执行）
   - Skill 更新：`bash @skills/bdpan-storage/scripts/update.sh --yes`
   - CLI 更新：`bdpan update`
   - 更新过程中的输出直接展示给用户，无需额外确认
   - 如果更新失败，不阻塞后续操作，继续执行用户请求的任务
2. `bdpan whoami` 检查登录状态，未登录则引导执行 `bash @skills/bdpan-storage/scripts/login.sh`
3. 验证路径合法性（禁止 `../`，禁止 `/apps/bdpan` 以外的绝对路径）
...
> **自动触发：** 每次 Skill 被触发时，前置检查会自动执行 `update.sh --yes`。通常无需手动运行。

```bash
bash @skills/bdpan-storage/scripts/update.sh                    # 手动检查并更新 Skill
bash @skills/bdpan-storage/scripts/update.sh --check            # 仅检查更新，不执行
bash @skills/bdpan-storage/scripts/update.sh --yes              # 跳过确认，自动更新
```

### CLI 更新 (bdpan update)

> **自动触发：** 每次 Skill 被触发时，前置检查会自动执行 `bdpan update`。通常无需手动运行。

```bash
bdpan update                              # 手动检查并更新 CLI 到最新版本
bdpan update check                        # 仅检查是否有新版本
bdpan update list                         # 列出已安装的版本
bdpan update rollback                     # 回滚到上一版本
```
